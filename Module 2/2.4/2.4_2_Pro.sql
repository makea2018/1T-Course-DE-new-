...
-- Получаем полную таблицу по зарплатам, премиям отделов (имя: salary_table_final)
	salary_table as
(
	select *, "Уровень зарплаты" + ("Уровень зарплаты" * "Индексация") as "Зарплата после индексации"
	from	(select t1.*, case
				  when Премия < 1.1 then 0
				  when Премия <= 1.2 and Премия >= 1.1 then 0.1
				  when Премия > 1.2 then 0.2
			...
			from (select Кварталы.ФИО, "Уровень зарплаты", Премия 
				  from Кварталы
				  join Сотрудники on Сотрудники.Id = Кварталы.Id) as t1) as t2
),

	salary_table_final as
(
	select "Идентификатор отдела", sum("Уровень зарплаты") as "Сум. Зарплата до индексации", sum("Зарплата после индексации") as "Сум. Зарплата после индексации", sum("Премия") as "Сум. премия", avg("Индексация") * 100 as "Ср. размер премии, %"
	from (select "Идентификатор отдела", salary_table.*
		  from salary_table
		  right join Сотрудники on Сотрудники.ФИО = salary_table.ФИО) as help_t
	group by "Идентификатор отдела"
),
...
-- Итоговая таблица
	main_table as 
(
	select "Название отдела", "ФИО руководителя", Отделы."Количество сотрудников", "ср. стаж, дней", "ср. зарплата", "кол-во junior", "кол-во middle", "кол-во senior", "кол-во lead", "Сум. Зарплата до индексации", "Сум. Зарплата после индексации"
	from Отделы
	...
	left join salary_table_final on Отделы."Идентификатор отдела" = salary_table_final."Идентификатор отдела"
)
